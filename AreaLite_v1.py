...
def browse_and_save_files():
    arealite_input = browse_file("Select AREALITE_Delta File")
    cli_dump_input = browse_file("Select CLI_DUMP File")

    # Save the files as AREALITE_Delta.txt & CLI_DUMP.txt
    with open("AREALITE_Delta.txt", "w", encoding="utf-8") as f:
        f.write(open(arealite_input, encoding="utf-8").read())

    with open("CLI_DUMP.txt", "w", encoding="utf-8") as f:
        f.write(open(cli_dump_input, encoding="utf-8").read())

    return "AREALITE_Delta.txt", "CLI_DUMP.txt"
